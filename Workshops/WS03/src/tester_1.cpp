...
enum AppErrors {
  ...
  RecordParsingError =
      3, // Some unknown issue appeared when processing a record from file
};
...
void itemAddedObserver(const seneca::Collection &theCollection,
                       const seneca::MediaItem &theBook) {
...
int main(int argc, char **argv) {
...

  {
    ...
    ::loadMedia<seneca::TvShow>(col,
                                "tvShows.csv"); // should have no effect since
                                                // the shows are already loaded
    std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
  }
...

  {
    ...
    seneca::g_settings.m_maxSummaryWidth = 100;
    seneca::TvShow *show =
        dynamic_cast<seneca::TvShow *>(col["Agatha All Along"]);
    std::cout << *show;
...

  {
    ...
    std::cout << "==========:==========:==========:==========:==========\n";
    seneca::TvShow *show =
        dynamic_cast<seneca::TvShow *>(col["Game of Thrones"]);
...
void loadMedia(seneca::Collection &col, const char *filename) {
  ...
  for (std::string record{}; std::getline(file, record); /* nothing */) {
    try {
      ...
      std::cout << "ERROR: Unknown error when processing file record ["
                << record << "].\n";
